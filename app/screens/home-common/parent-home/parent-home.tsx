      ...
      <View
        ...
        {isBetaVersion ? (
          <RNText
            ...
            {i18n.language === 'en'
              ? `Dear parents, ${parentName}! We're extremely curious and excited to hear your feedback on TeeFi. It's just 3 questions. `
              : `Cha mẹ ${parentName} thân yêu! Những góp ý của bạn sẽ giúp nền tảng TeeFi tốt hơn mỗi ngày. Chỉ có 3 câu hỏi ngắn thôi. `}
            <RNText
              style={[
                DASHBOARD_ALERT_TEXT,
                {
      ...
      {!isTablet() && (
        <View style={CONTAINER_CONTENT}>
          <View style={DIRECTION_VIEW}>
            <Text text={`${t('welcome')} `} style={[WELCOME_TEXT, {flex: 0}]} />
            <Text
              text={`${parentName}!`}
              style={WELCOME_TEXT}
              numberOfLines={1}
...
const CONTAINER_CONTENT: ViewStyle = {
  ...
  marginTop: isTablet() ? verticalScale(-10) : verticalScale(-18),
};
...
const FOOTER_TEXT_COMMON: TextStyle = {
  fontFamily: typography.promptSemiBold,
  fontSize: isTablet() ? moderateScale(24) : moderateScale(16),
  fontWeight: '600',
  color: color.black1,
};
