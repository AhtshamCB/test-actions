...
import {BanIcon, CupIcon, CupIconTablet} from '@app/svg';
...
export const LeaderboardSchool: FC<
  StackScreenProps<NavigatorParamList, 'leaderboardSchool'>
> = () => {
  const {leaderboardInfo} = useSelector(selector.dashboard);
  const {orientationOpenApp} = useSelector(selector.config);
  const [refreshing, setRefreshing] = useState<boolean>(false);

  const {getListLeaderboard, isLoading} = useSchool();
  ...
  return (
    <ScrollView
      ...
      <View>
        {leaderboardInfo && leaderboardInfo?.length > 0 ? (
          <>
              ...
              <React.Fragment key={index}>
                {index === 0 ? (
                  <LinearGradient
                    ...
                    <FastImage source={{uri: item?.avatar}} style={AVATAR} />
                    <Text
                      text={item.name}
                      style={[
                        NAME_TEXT,
...
const CONTENT_CONTAINER: ViewStyle = {
  alignItems: 'center',
  marginTop: isTablet() ? verticalScale(0) : verticalScale(-2),
};
